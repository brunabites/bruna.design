...
const SocialShare = ({ title, slug }: SocialShareProps) => {
	const socialShareLinks = [
		{
			href: `https://twitter.com/intent/tweet?text=${title}&url=https://bruna.design/posts/${slug}`,
			label: 'Twitter',
			icon: IoLogoTwitter,
			bg: '#1DA1F2',
		},
		{
			href: `https://www.facebook.com/sharer/sharer.php?u=https://bruna.design/posts/${slug}`,
			label: 'Facebook',
			icon: IoLogoFacebook,
			bg: '#4267B2',
		},
		{
			href: `https://www.linkedin.com/shareArticle?url=https://bruna.design/posts/${slug}&title=${title}`,
			label: 'LinkedIn',
			icon: IoLogoLinkedin,
			bg: '#2867B2',
		},
		...
		<div className="py-8">
			...
			<div className="flex items-center justify-start flex-nowrap">
				{socialShareLinks.map((link, index) => {
					return (
						<ExternalLink key={index} className="mr-8 last:mr-0 hover:no-underline" href={link.href}>
							<div
